 ...
 * (ds4_expert_cache).  Synthetic tables, no GGUF required: exercises
 ...
 *
 * Build:  make CUDA_ARCH=sm_120a tests/ds4_expert_cache_test
 * Run:    ./tests/ds4_expert_cache_test        (needs a CUDA device)
 */
#include "ds4_gpu.h"
...
static int32_t lookup_or_neg(uint32_t layer, uint32_t expert) {
    int32_t slot = -2;
    if (!ds4_expert_cache_lookup(layer, expert, &slot))
        return -2;
    return slot;
}
...
int main(void) {
    printf("expert-cache unit test: tables=%u layers=%u count=%u row=%llu\n",
    ...
    ds4_expert_table tables[N_TABLES];
    for (uint32_t t = 0; t < N_TABLES; t++) {
    ...
    check(!ds4_expert_cache_configure(tables, N_TABLES, map, N_TABLES * ROW - 1),
          "configure refuses budget smaller than one slot");
    const uint64_t budget = (uint64_t)SLOTS * N_TABLES * ROW + 123;
    check(ds4_expert_cache_configure(tables, N_TABLES, map, budget), "configure succeeds");
    check(ds4_expert_cache_enabled(), "enabled after configure");
    check_u(ds4_expert_cache_slot_count(), SLOTS, "slot count = budget/(tables*row)");

    int32_t slots[COUNT];
    const int32_t ids_a[] = {1, 2, 3};

    /* 2. First stage loads all three; slots distinct and in range. */
    check(ds4_expert_cache_stage(0, ids_a, 3, slots), "stage layer0 loads 3 ids");
    int ok = 1;
    ...
    check(ds4_expert_cache_stage(0, ids_a, 3, slots), "restage same ids succeeds");
    ...
    for (uint32_t i = 0; i < COUNT; i++)
        many[i] = (int32_t)i;
    check(!ds4_expert_cache_stage(0, many, COUNT, slots),
    ...
    check(ds4_expert_cache_stage(1, layer1_ids, SLOTS, slots),
    ...
    check(ds4_expert_cache_stage(1, layer1_ids2, SLOTS, slots),
    ...
    check(!ds4_expert_cache_enabled(), "disabled after shutdown");
    check(!ds4_expert_cache_stage(0, ids_a, 3, slots), "stage fails after shutdown");
    ds4_expert_cache_shutdown();
    check(1, "double shutdown safe");

    /* 7. Reconfigure with a huge budget clamps slots to the expert count. */
    check(ds4_expert_cache_configure(tables, N_TABLES, map, 1024ull * 1024ull * 1024ull),
          "reconfigure with huge budget");
    check_u(ds4_expert_cache_slot_count(), COUNT, "slots clamp to expert count");
    ds4_expert_cache_shutdown();
